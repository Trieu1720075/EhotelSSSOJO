...
eTVPlayer.timeToSec = function(time){
	//eCommon.logs("eTVPlayer.timeToSec: "+ time);
	var sec = (time+"").split(':');
	//eCommon.logs("eTVPlayer.timeToSec: "+ JSON.stringify(sec));
...
eTVPlayer.successCB = function(avplay) {
	...
	var playCB = {
		oncurrentplaytime : function(time) {
			/*var sec = eTVPlayer.timeToSec(time)+1;
			var cur = eTVPlayer.playTime/2;
			//eCommon.logs("oncurrentplaytime: "+cur+":"+sec);
			if (sec > 0 && cur > sec) {
				if (eTVPlayer.status != "oncurrentplaytime"){
					eCommon.loading();
					eTVPlayer.status = "oncurrentplaytime";
					eTVPlayer.playTime = 0;
					eCommon.logs("oncurrentplaytime");
				}
			} else if (sec - cur > 5) {
				eTVPlayer.playTime = sec*2-1;
			} else {
				eTVPlayer.playTime++;
			}*/
		},
		onresolutionchanged : function(width, height) {
			...
			//eTVPlayer.status = "onresolutionchanged";
		},
		onstreamcompleted : function() {
			eCommon.logs("onstreamcompleted");
			//eTVPlayer.status = "onstreamcompleted";
			/*eTVPlayer.stop();
			eTVPlayer.AVPlayer(eTVPlayer.url);*/
		},
		onerror : function(e) {
			//eTVPlayer.status = "onerror";
			eCommon.logs(JSON.stringify({
				error: e.name,
	...
	var bufferingCB = {
		...
		onbufferingprogress : function(percent) {
			...
			/*if (eTVPlayer.playTime > 0){
				eCommon.loading();
				eTVPlayer.stop();
				eTVPlayer.AVPlayer(eTVPlayer.url);
			}
			eTVPlayer.status = "onbufferingprogress";*/
		},
...
eTVPlayer.playAV = function(){
	...
	try{
		...
		eTVPlayer.avPlayer.open(_.url, {totalBufferSize: 10485760, pendingBufferSize: 1048576});
...
eTVPlayer.onErrorKeyDown = function(event) {
	event.preventDefault();
	var _ = this;
	var keyCode = event.keyCode;
	switch (keyCode) {
